$(function(){
	...
	window.drawNumber = (number) => {
		...
		$(this).delay(intialDelay).queue((next)=> {
			$(".p0 g").removeClass('move');
			$(".p0 .b"+(digits[0]%5)+", .p0 .t"+Math.floor(digits[0]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p1 g").removeClass('move');
			$(".p1 .b"+(digits[1]%5)+", .p1 .t"+Math.floor(digits[1]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p2 g").removeClass('move');
			$(".p2 .b"+(digits[2]%5)+", .p2 .t"+Math.floor(digits[2]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p3 g").removeClass('move');
			$(".p3 .b"+(digits[3]%5)+", .p3 .t"+Math.floor(digits[3]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p4 g").removeClass('move');
			$(".p4 .b"+(digits[4]%5)+", .p4 .t"+Math.floor(digits[4]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p5 g").removeClass('move');
			$(".p5 .b"+(digits[5]%5)+", .p5 .t"+Math.floor(digits[5]/5)).addClass('move');
			next();
		}).delay(stepDelay).queue((next) => {
			$(".p6 g").removeClass('move');
			$(".p6 .b"+(digits[6]%5)+", .p6 .t"+Math.floor(digits[6]/5)).addClass('move');
			next();
		});
	}
...
function validateLogIn() {
	...
	} else if (!/^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/.test(email) && email!="admin") {
		alert("Invalid email!");
	} else {
		return true;
	}
